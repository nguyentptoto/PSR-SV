...
@section('content')
    ...
    @if (empty($importedPurchaseRequests))
        <div class="alert alert-warning">
            Không có phiếu đề nghị nào hợp lệ được tìm thấy để xem trước. Vui lòng quay lại trang <a href="{{ route('users.purchase-requests.create') }}">Tạo phiếu mới</a> và import lại.
        </div>
    @else
        <p>Tìm thấy <strong>{{ count($importedPurchaseRequests) }}</strong> phiếu đề nghị từ file Excel.</p>
        <p class="text-danger">Vui lòng kiểm tra kỹ thông tin bên dưới trước khi tạo phiếu. Bạn có thể chỉnh sửa các trường trước khi tạo.</p>
        ...
        <form id="create-from-import-form" action="{{ route('users.purchase-requests.create-from-import') }}" method="POST">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="{{ route('users.purchase-requests.create') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Quay lại Import File khác</a>
            </div>
            ...
            @foreach ($importedPurchaseRequests as $prIndex => $prData)
                <div class="card card-outline card-primary mb-4 pr-preview-card" data-pr-index="{{ $prIndex }}">
                    <div class="card-header">
                        <h3 class="card-title"><strong>Phiếu #{{ $prIndex + 1 }} - Mã PR: {{ $prData['pia_code'] }}</strong></h3>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-4 form-group">
                                ...
                                <input type="text" name="prs[{{ $prIndex }}][pia_code]" class="form-control" value="{{ $prData['pia_code'] }}" required readonly>
                            </div>
                            <div class="col-md-4 form-group">
                                <label>Phòng ban của bạn</label>
                                <input type="text" class="form-control" value="{{ $user->sections->first()->name ?? 'N/A' }}" disabled>
                                <input type="hidden" name="prs[{{ $prIndex }}][section_id]" value="{{ $user->sections->first()->id ?? '' }}">
                            </div>
                            <div class="col-md-4 form-group">
                                <label>Phòng ban yêu cầu</label>
                                <select name="prs[{{ $prIndex }}][executing_department_id]" class="form-control select2-basic" style="width: 100%;" required>
                                    @foreach ($executingDepartments as $department)
                                        <option value="{{ $department->id }}" {{ $prData['executing_department_id'] == $department->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4 form-group">
                                ...
                                <input type="text" class="form-control" value="{{ $user->mainBranch->name ?? 'N/A' }}" disabled>
                                <input type="hidden" name="prs[{{ $prIndex }}][branch_id]" value="{{ $user->mainBranch->id ?? '' }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-4 form-group">
                                <label>Ngày phát hành (SAP)</label>
                                <input type="date" name="prs[{{ $prIndex }}][sap_release_date]" class="form-control" value="{{ $prData['sap_release_date'] }}">
                            </div>
                            <div class="col-md-4 form-group">
                            ...
                            <div class="col-md-4 form-group">
                                ...
                                <select name="prs[{{ $prIndex }}][currency]" class="form-control" required>
                                    <option value="VND" {{ $prData['currency'] == 'VND' ? 'selected' : '' }}>VND</option>
                                    <option value="USD" {{ $prData['currency'] == 'USD' ? 'selected' : '' }}>USD</option>
                                </select>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-4 form-group">
                                ...
                                <input type="date" name="prs[{{ $prIndex }}][sap_request_date]" class="form-control" value="{{ $prData['sap_request_date'] }}">
                            </div>
                            <div class="col-md-4 form-group">
                                <label>Số PO (PO Number)</label>
                                <input type="text" name="prs[{{ $prIndex }}][po_number]" class="form-control" value="{{ $prData['po_number'] }}">
                            </div>
                            <div class="col-md-4 form-group">
                                <label>Ngày PO (PO Date)</label>
                                <input type="date" name="prs[{{ $prIndex }}][po_date]" class="form-control" value="{{ $prData['po_date'] }}">
                            </div>
                            <div class="col-md-4 form-group">
                                <label>Người tạo (SAP Created By)</label>
                                <input type="text" name="prs[{{ $prIndex }}][sap_created_by]" class="form-control" value="{{ $prData['sap_created_by'] }}">
                            </div>
                        </div>
                                ...
                                <tbody>
                                    @foreach ($prData['items'] as $itemIndex => $item)
                                        <tr>
                                            ...
                                            <td><input type="number" name="prs[{{ $prIndex }}][items][{{ $itemIndex }}][r3_price]" class="form-control form-control-sm" value="{{ $item['r3_price'] }}" step="any" min="0"></td>
        ...
        $(document).ready(function () {
            ...
            function calculatePrTotals(prCard) {
                ...
                prCard.find('tbody tr').each(function () {
                    ...
                    row.find('.item-subtotal-display').val(subtotal.toLocaleString('vi-VN', {minimumFractionDigits: 2}));
                    currentPrGrandTotal += subtotal;
                    currentPrTotalOrderQty += orderQty;
                    currentPrTotalInventoryQty += inventoryQty;
                });

                prCard.find('.pr-total-amount-display').text(currentPrGrandTotal.toLocaleString('vi-VN', {minimumFractionDigits: 2}));
                prCard.find('.pr-total-order-quantity-display').text(currentPrTotalOrderQty.toLocaleString('vi-VN', {minimumFractionDigits: 3}));
                prCard.find('.pr-total-inventory-quantity-display').text(currentPrTotalInventoryQty.toLocaleString('vi-VN', {minimumFractionDigits: 3}));
            }

            // Gắn sự kiện input cho các trường số lượng/giá trong mỗi phiếu
            $('#create-from-import-form').on('input', '.item-quantity-input, .item-price-input, .item-inventory-quantity-input', function () {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    Swal.fire({
                                        title: 'Thành công!',
                                        html: response.message + (response.errors.length > 0 ? '<br><br><strong>Chi tiết cảnh báo:</strong><br>' + response.errors.join('<br>') : ''),
                                        icon: 'success',
                                    ...
                                    Swal.fire({
                                        title: 'Lỗi!',
                                        html: 'Đã xảy ra lỗi khi tạo phiếu:' + errorHtml,
                                        icon: 'error'
                                    });
                                }
                            },
            ...
            if (importedMessages.length > 0) {
                ...
                importedMessages.forEach(msg => {
                    if (msg.type === 'error') {
                        html += `<div class="alert alert-danger alert-dismissible"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>${msg.text}</div>`;
                    } else if (msg.type === 'warning') {
                        html += `<div class="alert alert-warning alert-dismissible"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>${msg.text}</div>`;
                    }
                });
